...
def test_symmetric() -> None:
    ...
    """
    for _a in [-10.0, -1.0, 0.0, 1.0, 10.0]:
        for _b in [-10.0, -1.0, 0.0, 1.0, 10.0]:
            assert_close(mul(_a, _b), mul(_b, _a))
...
def test_distribute() -> None:
    ...
    for _a in [-10.0, -1.0, 0.0, 1.0, 10.0]:
        for _b in [-10.0, -1.0, 0.0, 1.0, 10.0]:
            for _c in [-10.0, -1.0, 0.0, 1.0, 10.0]:
                assert_close(mul(_a, add(_b, _c)), add(mul(_a, _b), mul(_a, _c)))
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    assert_close(sum, minitorch.operators.sum(ls1) + minitorch.operators.sum(ls2))
